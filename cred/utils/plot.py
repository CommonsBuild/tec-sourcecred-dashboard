...
def pie_chart(data: pd.Series,
    ...
    data = data.reset_index(name='value').rename(columns={'index': index_name})
    data['angle'] = data['value']/data['value'].sum() * 2*pi
    data['perc'] = data['value']/data['value'].sum() * 100
    if colors:
        data['color'] = colors
    else:
        data['color'] = Category20c[len(data)] if len(data) > 2 else Category20c[3][:data.shape[0]]

    p = figure(plot_height=plot_height,
               title=title, toolbar_location=toolbar_location,
               tools="pan,save,hover", tooltips=f"@{index_name}: @value (@perc%)",
               x_range=x_range,
              )
